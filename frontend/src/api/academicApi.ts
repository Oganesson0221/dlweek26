...
export async function getCourseOutline(courseCode: string): Promise<DbCourseOutline | null> {
...
export async function getCourseComponents(courseCode: string): Promise<DbCourseComponent[]> {
...
export async function getCourseTopics(courseCode: string): Promise<DbTopicNode[]> {
  const res = await apiClient.get(`/academic/courses/${courseCode}/topics`);
  return res.data;
}

export async function listAssignments(courseCode: string): Promise<DbAssignment[]> {
  const res = await apiClient.get(`/academic/submissions/courses/${courseCode}/assignments`);
  return res.data.assignments ?? res.data;
}

export async function getAssignmentReminders(assignmentId: number): Promise<DbReminder[]> {
  const res = await apiClient.get(`/academic/submissions/assignments/${assignmentId}/reminders`);
  return res.data.reminders ?? [];
}

export async function getAssignmentWorkplan(assignmentId: number): Promise<DbWorkPlan | null> {
  const res = await apiClient.get(`/academic/submissions/assignments/${assignmentId}/workplan`);
  return res.data.workplan ?? null;
}

export async function createAssignment(courseCode: string, body: {
  title: string;
  description?: string;
  due_at: string;
  weight: number;
}) {
  const res = await apiClient.post(`/academic/submissions/courses/${courseCode}/assignments`, body);
  return res.data;
}
...
export async function generateWord(assignmentId: number) {
  const res = await apiClient.post(`/academic/templates/word`, { assignment_id: assignmentId });
  return res.data;
}

export async function generatePpt(assignmentId: number) {
  const res = await apiClient.post(`/academic/templates/ppt`, { assignment_id: assignmentId });
  return res.data;
}
