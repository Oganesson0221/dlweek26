import type { Course, CourseOutlineComponent, CourseTopic, Assignment, Checkpoint, CheckpointType } from "@/types";
import type { DbCourse, DbCourseOutline, DbCourseComponent, DbTopicNode, DbAssignment } from "@/types/backendAcademic";

// helper: pick a consistent id scheme for frontend (string)
const cid = (code: string) => code.toLowerCase();

function mapComponents(components: DbCourseComponent[], outlineText?: string): CourseOutlineComponent[] {
...
function mapAssignments(courseCode: string, courseId: string, items: DbAssignment[]): Assignment[] {
