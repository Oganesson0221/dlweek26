...
(function() {
  ...
  async function handleSaveClick(e) {
    ...
    try {
      ...
      floatingBtn.style.background = 'linear-gradient(135deg, #107c10, #16a34a)';
      showNotification('Note saved to Clippy! 📝');
    } catch (error) {
      console.error('Error saving note:', error);
      floatingBtn.innerHTML = '<span>❌ Error</span>';
      floatingBtn.style.background = 'linear-gradient(135deg, #d13438, #ef4444)';
      showNotification('Failed to save note. Make sure the backend is running.');
    }
    
    // Reset and hide button after delay
    setTimeout(() => {
      floatingBtn.innerHTML = originalContent;
      floatingBtn.style.background = 'linear-gradient(135deg, #0078d4, #50a0e0)';
      hideFloatingButton();
    }, 1500);
  }
